//@ts-ignore
import fs from "fs";
const lines: string[] = fs.readFileSync("./day7/t.txt", "utf-8").split(/\n/).filter(Boolean);

const cardValues1 = ["A", "K", "Q", "J", "T", "9", "8", "7", "6", "5", "4", "3", "2"] as const;
const cardValues2 = ["A", "K", "Q", "T", "9", "8", "7", "6", "5", "4", "3", "2", "J"] as const;

const run = (sol2: boolean) => {
  ...
  const data = lines.map((l) => {
    ...
    const cardVC = d[0].split("").reduce((acc, c) => {
      acc[c] = acc[c] + 1 || 1;
      return acc;
    }, {} as Record<keyof typeof cardValues, number>);
